...
export async function getEventoById(id: number): Promise<EventoDetalle | null> {
  const evento = await prisma.evento.findFirst({
    ...
    select: {
      ...
      aforoPrevisto: true, aforoEstimado: true, aforoTotal: true,
      personalRiesgo: true, temporada: true,
...
export async function createEvento(input: CreateEventoInput): Promise<EventoDetalle> {
  const evento = await prisma.evento.create({
    data: {
      ...
      aforoEstimado: input.aforoEstimado ?? null,
      aforoTotal: input.aforoTotal ?? null,
      personalRiesgo: input.personalRiesgo ?? null,
...
export async function updateEvento(id: number, input: UpdateEventoInput): Promise<EventoDetalle> {
  ...
  await prisma.evento.update({
    ...
    data: {
      ...
      ...(input.aforoTotal !== undefined && { aforoTotal: input.aforoTotal }),
      ...(input.personalRiesgo !== undefined && { personalRiesgo: input.personalRiesgo }),
